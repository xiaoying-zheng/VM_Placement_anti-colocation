...
public class RandomAssignment
{
	...
	// constructors
	public RandomAssignment(String conf_VM, String conf_PM, String VM_file, String PM_file, boolean bFractional)
	{
		String line = null;
		N = 0;
		M = 0;
...
*/
	...
	public static void main(String[] args)
	{
		if(args.length != 4)
		{
			System.out.println("Usage: RandomAssignment VM_Set.num PM_Set.num output_model.lp beFractional(0/1).");
			System.exit(0);			
		}

		RandomAssignment solver = null;
		if(args[3].compareTo("0") == 0)
			solver = new RandomAssignment("conf/VM.conf", "conf/PM.conf", args[0], args[1], false);
		else
			solver = new RandomAssignment("conf/VM.conf", "conf/PM.conf", args[0], args[1], true);
...
//		solver.solve(args[2]);
		...
		{
			double obj = solver.solve(args[2]);
			if(obj != -1)
			{
				obj_sum += obj;
				count++;
		...
		System.out.println("Average obj = " + obj_sum / count);
	}
}
